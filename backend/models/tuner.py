...
class TunerStatusResponse(BaseModel):
    ...
    reason: Optional[str] = Field(default=None, description="Explanation for why the bottleneck was detected")
    recommended_action: Optional[str] = Field(default=None, description="Recommended or applied safe tuning action")
    observation_remaining_seconds: int = Field(default=0, description="Seconds remaining in observation window (30s)")
    cooldown_remaining_seconds: int = Field(default=0, description="Seconds remaining in cooldown period (30s)")
