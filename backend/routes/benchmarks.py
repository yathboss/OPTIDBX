...
def export_benchmark(identifier: str, format: str = 'json', service=Depends(get_benchmarks)):
    try:
        current = service.status()
        record = current if current and current['id'] == identifier else service.store.get(identifier)
    except FileNotFoundError as exc:
        raise HTTPException(404, 'Unknown benchmark') from exc
    except ValueError as exc:
        raise HTTPException(422, 'Invalid benchmark identifier') from exc
    if format == 'json':
        return JSONResponse(record, headers={'Content-Disposition': f'attachment; filename="benchmark-{record["id"]}.json"'})
    if format != 'csv':
        raise HTTPException(422, 'Choose json or csv')
    output = io.StringIO()
    fields = ['pair', 'mode', 'experiment_id', 'status', 'throughput_qps', 'median_latency_ms',
              'p95_latency_ms', 'successful_queries', 'errors', 'timeouts', 'elapsed_seconds']
