...
CRITERIA = {
    "minimum_pairs": 5,
    "minimum_measurement_seconds": 180,
    "minimum_warmup_seconds": 30,
    "minimum_successful_queries": 30,
    "throughput_improvement_percent": 10,
    "maximum_p95_degradation_percent": 5,
...
def interval(values):
    ...
    return {
        ...
        "interval_95": [percentile(estimates, 0.025), percentile(estimates, 0.975)],
    }
...
def evaluate(runs, config):
    result = {
        ...
        "reason": "No completed comparison yet.",
        "completed_pairs": 0,
        "throughput_change": None,
        "p95_change": None,
        "method": (
            "Paired percentile bootstrap, 5000 resamples, 95% interval; "
            "exploratory for small samples."
        ),
    }
    if not runs:
        return result
    ...
    for pair in pairs:
        ...
        if any(
            not m.get("throughput_qps")
            or not m.get("p95_latency_ms")
            or m.get("overflow")
    ...
    if (
        len(pairs) < CRITERIA["minimum_pairs"]
        or config.get("warmup_seconds", 0) < CRITERIA["minimum_warmup_seconds"]
        or any(
            row["metrics"]["elapsed_seconds"] < CRITERIA["minimum_measurement_seconds"]
            for row in runs
        )
    ):
        result["reason"] = (
            "Pilot only: at least 5 pairs, 30s warm-up and 180s measurement per run are required."
        )
        return result
    higher_errors = any(
        pair["adaptive"]["metrics"].get("error_rate", 0)
        > pair["baseline"]["metrics"].get("error_rate", 0)
        for pair in pairs
    )
    if (
        result["throughput_change"]["interval_95"][1] < -CRITERIA["throughput_regression_percent"]
        or result["p95_change"]["interval_95"][0] > CRITERIA["maximum_p95_degradation_percent"]
        or higher_errors
    ):
        result.update(
            verdict="REGRESSION_OBSERVED",
            reason="Throughput, tail latency or error rate regressed under the frozen criteria.",
        )
    elif (
        result["throughput_change"]["interval_95"][0] >= CRITERIA["throughput_improvement_percent"]
        and result["p95_change"]["interval_95"][1] <= CRITERIA["maximum_p95_degradation_percent"]
        and any(
            action.get("verified_applied_value") is not None
            for pair in pairs
        ...
        result.update(
            ...
            reason=(
                "Repeated measured improvement met the frozen criteria for this workload and host."
            ),
        )
    else:
        result["reason"] = (
            "Intervals do not establish the required benefit, or no automatic change was applied."
        )
    return result
