...
class BenchmarkService:
    def __init__(self, manager, store=None):
        ...
        for record in self.store.list():
            if record["status"] in ("RUNNING", "CANCELLING"):
                record.update(
                    status="INTERRUPTED",
                    error="API restarted; original comparison cannot resume.",
                    ended_at=utc(),
                )
        ...
        with self.manager.lock, self.lock:
            ...
            self.record = {
                ...
                "manifest": {
                    ...
                    "query": QUERY,
                    "shared_config": self.runtime.config.model_dump(mode="json"),
                    "measurement": (
                        "Client-observed query round trip including action barrier waits; "
                        "completed queries fully inside the fixed window; warm-up excluded."
                    ),
                },
            }
            try:
                ...
                self.record["evaluation"] = {
                    ...
                    "reason": self.record["error"],
                }
                raise
            self.manager.reservation = identifier
            self.cancel_event.clear()
            self.thread = threading.Thread(
                target=self._run, daemon=True, name="PerformanceEvidence"
            )
            self.thread.start()
            return self.status()
    ...
    def _wait(self, seconds, phase):
        ...
        while time.monotonic() < deadline:
            ...
            self._progress(
                phase=phase,
                remaining_seconds=max(0, round(deadline - time.monotonic(), 1)),
                measurements=self.manager.status().get("measurements"),
                tuner_state=self.runtime.lifecycle.status()["state"],
            )
            self.cancel_event.wait(min(0.5, max(0, deadline - time.monotonic())))
        ...
        try:
            ...
            try:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT version(), current_database(), count(*) FROM pgbench_accounts"
                    )
                    version, database, rows = cur.fetchone()
                with self.lock:
                    self.record["manifest"].update(
                        postgresql=version, database=database, dataset_rows=rows
                    )
            finally:
                conn.close()
            self._save()
            for position, spec in enumerate(self.record["order"]):
                ...
                self._progress(
                    **spec,
                    ...
                    total_runs=len(self.record["order"]),
                    phase="STARTING",
                )
                current_run = {
                    **spec,
                ...
                self.manager.start(
                    cfg["profile"],
                    cfg["warmup_seconds"] + cfg["measurement_seconds"],
                    owner=identifier,
                    initial_parallelism=cfg["initial_parallelism"],
                ...
                with self.runtime._lock:
                    ...
                    self.runtime.set_mode(
                        "auto" if spec["mode"] == "adaptive" else "recommendation"
                    )
                ...
                actions = [
                    action
                    for action in self.runtime.get_action_history()
                ...
                with self.lock:
                    current_run.update(
                        metrics=metrics, actions=actions, status="COMPLETED", ended_at=utc()
                    )
                self._save()
            with self.lock:
            ...
            try:
                ...
                if self.manager.connections or self.runtime.lifecycle.status()["recovery_required"]:
                    raise RuntimeError(
                        self.manager.error or "Rollback unresolved; sessions retained"
                    )
            except Exception as exc:
                with self.lock:
            ...
            with self.lock:
                ...
                try:
                    ...
                    self.record.update(
                        status="FAILED", error=f"Evidence storage failed: {type(exc).__name__}"
                    )
                    self.record["evaluation"] = {
                        "verdict": "INCONCLUSIVE",
                        "reason": self.record["error"],
