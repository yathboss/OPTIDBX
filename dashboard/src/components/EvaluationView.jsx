...
export default function EvaluationView({experiments, error}) {
  ...
  return <section>
    ...
    {detail && <div><h3>Run #{detail.id}: {detail.overall_result}</h3>
      <ReportButton run={detail}/><p>Whole-run averages: latency {detail.aggregate_metrics?.query_latency_ms?.toFixed(2) ?? 'Unavailable'} ms | throughput {detail.aggregate_metrics?.throughput_tps?.toFixed(2) ?? 'Unavailable'} TPS</p>
      {detail.actions?.length ? detail.actions.map(action => <BeforeAfterCard key={action.action_id} action={action} runMeta={detail}/>) : <p>No tuning action was applied in this run.</p>}
